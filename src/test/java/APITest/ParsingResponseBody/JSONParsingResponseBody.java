package APITest.ParsingResponseBody;
...
public class JSONParsingResponseBody {
    ...
    void checkBookTitlePresentOrNot() {
        ...
        Assert.assertEquals(status, true, "Books not found");
    }
    ...
    //Suppose we wanna check multiple values
    @Test(priority = 6)
    void hasValues() {
        given()
                .contentType("ContentType.JSON")
                ...
                .get("GivetheUrl")
                .then()
                .body("book[0]", Matchers.hasItems("ABC", "Test", 120.90, "Narach"));
    }
}
